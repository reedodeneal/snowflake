...
def validateJwt(token):
    public_key = "-----BEGIN PUBLIC KEY-----\n"+ os.getenv('KEYCLOAK_PUBLIC_KEY').strip("\"") +"\n-----END PUBLIC KEY-----"
    try:
        access_token_json = jwt.decode(token, public_key, audience='snowflake')
        return True
    except:
    ...
    def post(self):
        try:
            ...
            if auth_header.startswith("Bearer"):
                if validateJwt(auth_header.split(" ")[1]):
                    try:
